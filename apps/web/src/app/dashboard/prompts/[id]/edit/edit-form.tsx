...
export function EditPromptForm({ prompt }: Props) {
  const router = useRouter()
  const { getToken } = useAuth()
  const [title, setTitle] = useState(prompt.title)
  const [systemPrompt, setSystemPrompt] = useState(prompt.systemPrompt)
  const [variables, setVariables] = useState<Variable[]>(
    prompt.variables.map((v) => ({ name: v.name, label: v.label, defaultValue: v.defaultValue ?? '' })),
  )
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${workerUrl}/api/prompts/${prompt.id}`, {
        method: 'PATCH',
        headers: {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-900 mb-8">Edit Prompt Template</h1>
